#!/usr/bin/env python3
"""Batch generate kapiko song pages for ALL Billboard Year-End Hot 100 (2000-2020).

Reads top-100-by-year-2000-2020.json, skips existing pages, generates missing ones.
Uses Spotify Client Credentials for track metadata. Audio Features API is deprecated.
"""
...
DATA_FILE = Path.home() / 'kapiko-site' / 'top-100-by-year-2000-2020.json'
...
# ── Genre rules: artist/title keyword → genre ─────────────────────────────
GENRE_KEYWORDS_ARTIST = {
    'r-n-b': [
        'aaliyah','destiny','beyonce','usher','alicia keys','mary j','r kelly',
        'ne-yo','chris brown','trey songz','ciara','ashanti','monica','brandy',
        'r. kelly','jagged edge','joe','mya','donell','blaque','toni braxton',
        'sisqo','keith sweat','maxwell','d angelo','next','dru hill','guy',
        'total','case','sade','jill scott','erykah','musiq','avant','ginuwine',
        'tweet','kelly rowland','amerie','mario','omarion','fantasia',
        'tank','pleasure p','lloyd','t-pain','lil mo','nivea','h.e.r.',
    ],
    'hip-hop': [
        'jay-z','jay z','eminem','nelly','ludacris','50 cent','lil wayne',
        'kanye','drake','j. cole','kendrick','chance','travis scott','post malone',
        'cardi b','megan thee','future','21 savage','lil baby','gunna','roddy',
        'snoop','dre','nas','dmx','ja rule','eve','fat joe','big pun','busta',
        'missy','timbaland','outkast','andre 3000','big boi','lil jon','t.i.',
        'young jeezy','gucci mane','waka','rick ross','meek mill','wale',
        'mac miller','kid cudi','big sean','childish gambino','logic','khalid',
        'lil pump','6ix9ine','blueface','dababy','polo g','juice wrld',
        'xxxtentacion','lil peep','young thug','offset','quavo','takeoff',
        'migos','rae sremmurd','fetty wap','desiigner','24kgoldn','jack harlow',
        'lil nas x','roddy ricch','pop smoke','mulatto','saweetie','doja cat',
    ],
    'country': [
        'garth brooks','kenny chesney','tim mcgraw','faith hill','shania twain',
        'carrie underwood','taylor swift','rascal flatts','dierks bentley',
        'brad paisley','luke bryan','blake shelton','florida georgia line',
        'jason aldean','eric church','miranda lambert','thomas rhett',
        'sam hunt','chris stapleton','kelsea ballerini','luke combs',
        'morgan wallen','dan + shay','kane brown','maren morris',
        'old dominion','brett eldredge','cole swindell','darius rucker',
        'zac brown','kenny rogers','dolly parton','george strait',
        'alan jackson','toby keith','lee ann womack','sugarland',
        'lady antebellum','lady a','little big town','the band perry',
        'maddie & tae','carly pearce','lee brice','lonestar','jo dee',
    ],
    'latin': [
        'santana','shakira','marc anthony','jennifer lopez','ricky martin',
        'enrique iglesias','daddy yankee','bad bunny','j balvin','maluma',
        'ozuna','anuel aa','nicky jam','pitbull','reggaeton','wisin',
        'yandel','don omar','prince royce','romeo santos',
    ],
    'rock': [
        'nickelback','linkin park','creed','matchbox','vertical horizon',
        '3 doors down','puddle of mudd','staind','seether','default',
        'hinder','shinedown','breaking benjamin','papa roach','disturbed',
        'godsmack','korn','system of a down','slipknot','evanescence',
        'foo fighters','coldplay','u2','green day','blink-182','sum 41',
        'good charlotte','simple plan','yellowcard','my chemical romance',
        'fallout boy','panic','paramore','30 seconds to mars',
        'one republic','train','maroon 5','imagine dragons',
    ],
    'pop': [
        'britney spears','backstreet boys','nsync','christina aguilera',
        'mandy moore','jessica simpson','pink','nelly furtado','kelly clarkson',
        'avril lavigne','hilary duff','lindsay lohan','jonas brothers',
        'selena gomez','demi lovato','miley cyrus','ariana grande',
        'katy perry','lady gaga','kesha','rihanna','beyonce',
        'justin bieber','one direction','5 seconds of summer',
        'ed sheeran','sam smith','meghan trainor','charlie puth',
        'shawn mendes','camila cabello','billie eilish','halsey',
        'olivia rodrigo','dua lipa','lizzo','ava max',
    ],
    'gospel': [
        'kirk franklin','toby mac','lecrae','switchfoot',
        'casting crowns','newsboys','skillet','dc talk',
        'matt redman','chris tomlin','hillsong',
    ],
}

GENRE_KEYWORDS_TITLE = {
    'country': [
        'whiskey','truck','hometown','boots','country road','southern',
        'prairie','hoedown','rodeo','cowboy',
    ],
    'hip-hop': [
        'trap','swag','drip','flex','gang','hustle','grind',
    ],
}

def infer_genre_from_song(title, artist):
    """Guess genre based on artist/title keywords. Default to 'pop'."""
    artist_lower = artist.lower()
    title_lower = title.lower()
    
    # Check artist against genre keyword lists
    for genre, keywords in GENRE_KEYWORDS_ARTIST.items():
        for kw in keywords:
            if kw in artist_lower:
                return genre
    
    # Check title against title keywords
    for genre, keywords in GENRE_KEYWORDS_TITLE.items():
        for kw in keywords:
...
def infer_instruments(genre, acousticness, instrumentalness, energy):
    base = {
        ...
        'gospel': ['organ','choir','electric bass'],
...
def main():
    ...
    # Get Spotify credentials from keychain
    ...
    # Load all billboard songs
    print(f"Loading billboard data from {DATA_FILE}...")
    data = json.loads(DATA_FILE.read_text())
    
    all_songs = []
    for year, songs in sorted(data['years'].items()):
        for s in songs:
            slug = make_slug(s['title'], s['artist'])
            all_songs.append({
                'year': year,
                'rank': s['rank'],
                'title': s['title'],
    ...
    print(f"Total billboard songs: {len(all_songs)}")
    
    # Filter to only missing
    missing = [s for s in all_songs if not (SONGS_DIR / s['slug'] / 'index.html').exists()]
    existing_count = len(all_songs) - len(missing)
    print(f"Already existing: {existing_count}")
    print(f"To generate: {len(missing)}")
    
    if not missing:
        print("Nothing to do — all pages already exist!")
        return
    ...
    for i, song in enumerate(missing):
        ...
        print(f"[{i+1}/{len(missing)}] {year} #{rank}: {title} — {artist}")
        
        # Get primary artist for search
        search_artist = re.split(r'\s+feat\w*\s+|\s+ft\.?\s+|\s+featuring\s+|\s+with\s+', artist, flags=re.IGNORECASE)[0].strip()
        
        # Search Spotify
        track = search_track(title, search_artist, token)
        
        if not track:
            print(f"  WARN: Not found on Spotify, using fallback metadata")
            # Use metadata directly from billboard data
            track_data = {
                'name': title,
                'artists': artist,
                'slug': slug,
        ...
        # Determine genre using inference
        genre = infer_genre_from_song(title, artist)
        
        # Generate HTML
        try:
            html = make_page(track_data, genre)
            out_dir.mkdir(parents=True, exist_ok=True)
            (out_dir / 'index.html').write_text(html, encoding='utf-8')
            created += 1
            if created % 25 == 0:
                print(f"  ✓ Progress: {created} created so far...")
        except Exception as ex:
            print(f"  ERROR generating page: {ex}")
            import traceback; traceback.print_exc()
            errored += 1
            errors.append(f"{year} #{rank}: {title} — {artist}: {ex}")
        
        # Rate limit: 200ms between Spotify requests
        time.sleep(0.2)
    ...
    print("="*60)
    print(f"Total billboard songs: {len(all_songs)}")
    print(f"Already existed (skipped): {existing_count + skipped}")
